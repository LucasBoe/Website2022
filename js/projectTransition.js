...
function attachClickBehaviour() {
  $(".project-preview").click(function () {
    document.location.hash = "p:" + $(this).attr("id");
    tryProcessLocationHashAndOpenProjet();
    return false;
  });
...
function tryProcessLocationHashAndOpenProjet() {
  var hash = document.location.hash;
  if (~hash.indexOf("p:")) {
    var id = hash.slice(3);
    openProject($("#projectList").find("[id='" + id + "']"), id);
  }
}
